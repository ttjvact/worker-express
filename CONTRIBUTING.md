...
- Issues はバグ報告、機能提案、または対応が確定した作業タスクに使います。
- 1つの Issue では 1つの論点だけを扱い、複数の課題を混ぜないようにします。
- 実装前に仕様や優先度の整理が必要な話題は、先に Discussions で扱います。
- Discussions は使い方の質問、API 提案、MVP 範囲の相談など、結論が未確定の話題に使います。
- 実作業に入る Pull Request は、関連 Issue または Discussions を本文から参照できる状態にします。
- Pull Request には、目的、変更点、利用者影響、確認した項目を簡潔に書きます。
- 破壊的変更や Express 互換性に影響する変更は、Pull Request 本文で明示します。
